...
interface DiscountParams {
  basePrice: number;
  itemCategory: 'Select Category' | 'Don Macchiatos' | 'Pizza' | 'Drinks' | 'Dessert' | 'Combo';
}

export function calculatePWDDiscount({ basePrice, itemCategory }: DiscountParams) {
...
function PwdPage() {  
  ...
  const [selectedCategory, setSelectedCategory] = React.useState<'Select Category' | 'Don Macchiatos' | 'Pizza' | 'Drinks' | 'Dessert' | 'Combo'>('Select Category');
  ...
  const handleCalculateDiscount = () => {
    const result = calculatePWDDiscount({
      basePrice: price,
      itemCategory: selectedCategory as 'Select Category' | 'Don Macchiatos' | 'Pizza' | 'Drinks' | 'Dessert' | 'Combo'
    });
  ...
  const getInTouch = () => {
    window.location.href = 'mailto:support@donmacchiatos.com?subject=PWD Support Request';
  };
  ...
  return (
    ...
    <div className="min-h-screen p-8 bg-gradient-to-b from-amber-50 to-orange-100">
      ...
      <a href="/components/molecules/Home"><i className="fa-solid fa-arrow-left text-[black] text-[2rem] mt-[2rem] hover:text-[#FFA500]"></i></a>
        ...
        {/* Discount Calculator */}
        <section id='Discount_Calculator' popover='' className="mt-28 bg-[#ececec] p-8 rounded-lg shadow-md px-56">
            <h2 className="text-2xl font-semibold mb-6 text-black">Calculate PWD Discount</h2>
            <div className="space-y-4">
              <div> 
                <label className="block text-gray-700 mb-2">Item Category</label>
                <select 
                  className="w-full p-2 border rounded"
                  value={selectedCategory}
                  onChange={(e) => setSelectedCategory(e.target.value as 'Select Category' | 'Don Macchiatos' | 'Pizza' | 'Drinks' | 'Dessert' | 'Combo')}
              >
              ...
              {discountResult && (
                <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                  <h3 className="font-semibold text-lg mb-2">Discount Details:</h3>
                  <p>Original Price: ₱{discountResult.originalPrice.toFixed(2)}</p>
                  <p>Discount Amount: ₱{discountResult.discountAmount.toFixed(2)}</p>
                  <p>Final Price: ₱{discountResult.finalPrice.toFixed(2)}</p>
                  <p>Discount Percentage: {discountResult.discountPercentage}%</p>
                </div>
              )}
            </div>
      </section>
        ...
        <section className="mt-12 text-center">
          ...
          <button popoverTarget='Discount_Calculator' className="bg-[#FFA500] text-black font-medium  px-6 py-3 transition-all duration-300 ease-in-out rounded-lg hover:bg-orange-500 shadow-lg hover:shadow-orange-500/50 hover:text-white">
            Get in Touch
          </button>
        </section>
      </div>
