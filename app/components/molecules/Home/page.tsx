...
function MainPage() {
  const { getData, error, loading } = useGetDataDonMacchiatos(
            ...
            <div className="gap-4 md:gap-8 w-[95%] md:w-[90%] mx-auto md:ml-[7%] mt-[30px] md:mt-[50px] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              {product && product.length > 0 ? (
                product.map((data: Product, index: number) => {
                  ...
                  return (
                    <div
                      key={data.id || `product${index}`}
                      className="z-50 products w-full shadow-lg overflow-hidden rounded-[10px] mb-[20px] h-[auto] bg-[white] pb-[0.90rem] cursor-pointer duration-300 hover:scale-[1.02]"
                    >
                      <Image
                        src={imageUrl}
                      ...
                      <div className="px-4 md:px-6">
                        <h1 className="text-[1.3rem] mt-[0.90rem] text-[black] font-semibold">
                          {data.name}
                        </h1>
                        ...
                        <Link
                          ...
                          title="View more"
                          className="p-4 shadow-sm shadow-black  bg-gradient-to-b from-amber-50 to-orange-100 rounded-lg float-right my-2 duration-100 ease-in-out  hover:bg-gradient-to-b  hover:from-orange-100 hover:to-amber-50"
                        >
                          View more
                        </Link>
                      </div>
